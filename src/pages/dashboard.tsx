...
import { getWidgets } from 'apiSdk/widgets';
...
import { TravelPlanListPage as ShowCaseList } from 'pages/travel-plans';

function DashboardPage() {
  const { session } = useSession();

  const destinationStat = useSWR<PaginatedInterface<DestinationInterface>>(
    () => `/destinations`,
    () => getDestinations({ limit: 1, offset: 0 }),
  );

  const widgetStat = useSWR<PaginatedInterface<WidgetInterface>>(
    () => `/widgets`,
    () => getWidgets({ limit: 1, offset: 0 }),
  );

  const travelOptionStat = useSWR<PaginatedInterface<TravelOptionInterface>>(
    () => `/travel-options`,
    () => getTravelOptions({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Destinations',
      result: destinationStat?.data?.totalCount,
      isLoading: destinationStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Widgets',
      result: widgetStat?.data?.totalCount,
      isLoading: widgetStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Travel Options',
      result: travelOptionStat?.data?.totalCount,
      isLoading: travelOptionStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`travel-plans`} passHref legacyBehavior>
                  <Button
