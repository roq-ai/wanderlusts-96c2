...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  switch (req.method) {
    case 'GET':
      return getTravelOptions();
    case 'POST':
      return createTravelOption();
    default:
  ...
  async function getTravelOptions() {
    ...
    const response = await prisma.travel_option
      .withAuthorization({
        roqUserId,
      ...
      .findManyPaginated({
        ...convertQueryToPrismaUtil(query, 'travel_option'),
        take: limit,
        skip: offset,
        ...(order?.length && {
  ...
  async function createTravelOption() {
    ...
    const data = await prisma.travel_option.create({
      data: body,
    });
