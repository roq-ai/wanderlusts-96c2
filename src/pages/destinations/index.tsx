...
type ColumnType = ColumnDef<DestinationInterface, unknown>;

interface DestinationListPageProps {
...
export function DestinationListPage(props: DestinationListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getDestinations({
        relations: ['organization', 'travel_option.count', 'travel_plan.count'],
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<DestinationInterface>>(
    () => `/destinations?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: DestinationInterface) => {
    if (hasAccess('destination', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/destinations/view/${row.id}`);
    }
  };
  ...
  const columns: ColumnType[] = [
    ...
    hasAccess('travel_option', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          cell: ({ row: { original: record } }: any) => record?._count?.travel_option,
        }
      : null,
    ...
    <Box p={4} rounded="md" shadow="none">
      ...
      >
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="destination" />
        </Flex>

        {hasAccess('destination', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/destinations/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
